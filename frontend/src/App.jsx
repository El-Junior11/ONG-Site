...
// --- SIGNATURE ELEMENT: horizon-line divider with soft cloud silhouette ---
const HorizonDivider = ({ flip = false, className = "" }) => (
  <div
    aria-hidden="true"
  ...
  >
    <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="w-full h-full">
      <path d="M0,64 C120,90 220,40 340,58 C460,76 540,30 660,48 C780,66 860,32 980,50 C1080,64 1140,44 1200,56 L1200,120 L0,120 Z" fill="var(--paper)" />
      <path d="M0,50 C120,76 220,26 340,44 C460,62 540,16 660,34 C780,52 860,18 980,36 C1080,50 1140,30 1200,42" fill="none" stroke="var(--sky-bright)" strokeOpacity="0.35" strokeWidth="2" />
    </svg>
  </div>
);
  ...
  if (teamLoading) {
    return (
      <div className="py-20 text-center flex flex-col justify-center items-center bg-[var(--paper)]">
        <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[var(--sky)] mb-4"></div>
    ...
    <section className="py-16 bg-[var(--paper-light)]">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="mb-10 text-center md:text-left">
          <span className="text-[var(--sky)] font-mono-label font-bold uppercase tracking-[0.3em] text-xs">Staff & Expertise</span>
          <h3 className="text-2xl md:text-3xl font-display font-extrabold text-[var(--ink)] tracking-tight mt-1">
            Ceux qui font la <span className="bg-gradient-to-r from-[var(--sky)] to-[var(--sky-bright)] bg-clip-text text-transparent">différence</span>
          </h3>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {team.map((member) => (
            <div 
              key={member.id}
              className="group bg-white rounded-2xl p-4 shadow-[0_4px_20px_rgba(11,79,134,0.06)] hover:shadow-[0_12px_32px_rgba(11,79,134,0.14)] transition-all duration-300 border border-[var(--paper-line)] flex items-center gap-4 transform hover:-translate-y-1"
            >
              <div className="relative h-16 w-16 shrink-0 rounded-full overflow-hidden bg-[var(--sky-ice)] ring-2 ring-[var(--sky-bright)] ring-offset-2 ring-offset-white shadow-inner">
                <img 
                  src={member.img || `https://ui-avatars.com/api/?name=${encodeURIComponent(member.name)}&background=1C86C9&color=fff`} 
                  alt={member.name}
              ...
              <div className="min-w-0 flex flex-col justify-center">
                ...
                <p className="text-[10px] font-mono-label font-semibold text-[var(--sky)] uppercase tracking-wider mt-1">
                  {member.role}
                </p>
              </div>
            </div>
    ...
    <section className="py-20 bg-[var(--paper)] px-6 md:px-20 relative">
      <div className="container mx-auto">
        <div className="mb-16 text-left max-w-2xl">
          <span className="text-[var(--sky)] font-mono-label font-bold text-xs uppercase tracking-[0.4em]">Piliers d'intervention</span>
          <h2 className="text-3xl md:text-5xl font-display font-extrabold text-[var(--ink)] tracking-tighter mt-2">Notre méthodologie.</h2>
          <div className="h-1 w-20 bg-gradient-to-r from-[var(--sky)] to-[var(--sky-bright)] mt-4 rounded-full"></div>
        </div>
        ...
        <div className="space-y-24">
          {activities.map((item, index) => (
            <div key={index} className={`flex flex-col lg:items-center gap-10 md:gap-16 ${item.align === 'flex-row' ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
              <div className="w-full lg:w-1/2 overflow-hidden rounded-3xl shadow-xl shadow-[var(--sky-deep)]/10 border border-[var(--paper-line)]">
...
const ApproachSection = () => (
  <div className="py-16 bg-gradient-to-b from-[var(--paper-light)] to-[var(--sky-ice)] px-6 md:px-20 border-y border-[var(--paper-line)] relative overflow-hidden">
    <div className="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
      <div className="space-y-6 text-left">
        <div>
          <span className="text-[var(--sky)] font-mono-label font-bold text-xs tracking-[0.3em] uppercase">Philosophie</span>
          <h2 className="text-3xl font-display font-extrabold text-[var(--ink)] tracking-tighter mt-1">L'effort propre</h2>
        </div>
        <div className="space-y-4"> 
          <div className="bg-white p-6 rounded-2xl shadow-[0_4px_20px_rgba(11,79,134,0.06)] border border-[var(--paper-line)] transition-all hover:shadow-[0_12px_28px_rgba(11,79,134,0.12)]">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-[0_4px_20px_rgba(11,79,134,0.06)] border border-[var(--paper-line)] transition-all hover:shadow-[0_12px_28px_rgba(11,79,134,0.12)]">
              <p className="text-base font-display font-bold text-[var(--gold-dark)] mb-2 flex items-center gap-2">
                <HeartHandshake size={18} className="text-[var(--gold)]" /> Inclusion Totale
              </p>
      ...
      <div className="w-full">
        <div className="rounded-3xl overflow-hidden shadow-2xl shadow-[var(--sky-deep)]/15 border-4 border-white">
          <img src="/TENA.jpg" alt="Impact" className="w-full h-80 object-cover" />
        </div>
        <div className="mt-6 flex items-center gap-4 bg-white p-6 rounded-2xl shadow-[0_4px_20px_rgba(11,79,134,0.06)] border border-[var(--paper-line)]">
          <span className="text-4xl font-display font-extrabold bg-gradient-to-r from-[var(--sky)] to-[var(--sky-bright)] bg-clip-text text-transparent">100%</span>
          <div className="flex flex-col text-left">
            <span className="text-xs font-display font-bold uppercase tracking-widest text-[var(--ink)]">Démocratie Directe</span>
            <span className="text-[10px] font-mono-label font-bold text-[var(--sky)] uppercase">Consensus & Solidarité</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <>
      <div className="relative h-[70vh] bg-[var(--ink)] overflow-hidden text-left flex items-center">
        <div className="absolute inset-0 z-0">
          ...
          <div className="absolute bottom-0 left-1/4 w-72 h-72 rounded-full bg-[var(--gold)]/15 blur-3xl glow-blob" style={{ animationDelay: '3s' }}></div>
        </div>
        <div className="relative z-20 container mx-auto px-6 md:px-20 text-white max-w-5xl">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/10 border border-white/20 backdrop-blur-md text-[var(--sky-bright)] font-mono-label font-semibold text-xs rounded-full uppercase tracking-widest mb-5">
          ...
          <Link to="/contact" className="inline-flex items-center gap-3 bg-gradient-to-r from-[var(--sky)] to-[var(--sky-bright)] text-white px-8 py-4 font-mono-label font-bold uppercase text-xs tracking-widest hover:shadow-2xl hover:shadow-[var(--sky)]/40 hover:-translate-y-0.5 transition-all rounded-full shadow-lg shadow-[var(--sky)]/30">
...
function App() {
  ...
  if (loading) return (
    <>
      <DesignSystem />
      <div className="h-screen flex flex-col items-center justify-center bg-[var(--paper)] font-mono-label font-black uppercase text-xs tracking-widest text-[var(--sky)] gap-4">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[var(--sky)]"></div>
        Chargement...
      </div>
    </>
  );
  ...
  return (
    <Router>
      ...
      <div className="min-h-screen flex flex-col bg-[var(--paper)] text-[var(--ink)] relative font-body">
        ...
        <nav className="bg-white/80 backdrop-blur-xl sticky top-0 z-50 px-6 md:px-20 py-4 flex justify-between items-center border-b border-[var(--paper-line)] shadow-sm">
          <Link to="/" className="flex items-center space-x-3 text-left group">
            ...
            <span className="text-[var(--ink)] font-display font-extrabold text-sm md:text-base leading-tight tracking-tight">
              ONG TSINJO AINA <span className="text-[var(--sky)]">FIANARANTSOA</span><br />
          ...
          <div className="hidden md:flex items-center space-x-8">
            <ul className="flex space-x-8 text-xs font-mono-label font-extrabold uppercase tracking-widest text-[var(--ink-soft)] items-center">
              {navLinks.map((l) => (
                <li key={l.path}>
                  <NavLink 
                    to={l.path} 
                    className={({ isActive }) => isActive ? "text-[var(--sky)] border-b-2 border-[var(--sky)] pb-1" : "hover:text-[var(--sky)] transition-colors"}
                  >
                    {l.label}
                  </NavLink>
                </li>
            ...
            <div className="flex items-center space-x-4 border-l pl-6 border-[var(--paper-line)]">
              <Link to="/login" className="text-[var(--ink-soft)] hover:text-[var(--sky)] transition-colors p-2 rounded-full hover:bg-[var(--sky-ice)]">
                <UserCircle size={26} />
              </Link>
            </div>
          </div>
          ...
          <button className="md:hidden text-[var(--ink)] focus:outline-none p-2 rounded-xl hover:bg-[var(--sky-ice)] transition-colors" onClick={() => setIsMenuOpen(!isMenuOpen)}>
          ...
          <ul className="flex flex-col items-start py-6 space-y-2 px-4">
                ...
                <NavLink 
                  ...
                  className={({ isActive }) => 
                    `flex flex-row items-center gap-3 p-3 rounded-xl transition-all duration-200 w-full font-mono-label ${
                      isActive 
                        ? 'bg-[var(--sky-ice)] text-[var(--sky)] font-bold shadow-xs' 
                        : 'text-[var(--ink-soft)] hover:bg-[var(--sky-ice)]/70 hover:text-[var(--sky)]'
                    }`
                  }
                >
            ...
            <li className="mt-6 pt-4 border-t border-[var(--paper-line)] w-full">
              <Link 
                to="/login" 
                onClick={() => setIsMenuOpen(false)} 
                className="flex flex-row items-center gap-3 p-3 text-[var(--ink-soft)] hover:text-[var(--sky)] hover:bg-[var(--sky-ice)]/70 rounded-xl transition-colors font-mono-label"
              >
        ...
        <footer className="relative bg-gradient-to-b from-[var(--ink)] to-[var(--sky-deep)] pt-16 pb-6 text-center text-white/90 font-mono-label font-bold text-[10px] uppercase tracking-[0.4em] border-t-2 border-[var(--sky-bright)]/40 overflow-hidden">
          <HorizonDivider flip className="!bottom-auto !top-0" />
